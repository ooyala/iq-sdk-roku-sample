...
Function Init()
    ...
    'We create IQ object as a task node, defined in IQ.xml'
    m.iq = CreateObject("roSGNode","IQ")
    'We create an array to contain videoplayer events and send them to IQ'
...
Sub onVideoVisibleChange()
    ...
    'We send playerLoad event to IQ when videoPlayer is now visible'
    if m.videoPlayer.visible = true
        m.events["state"] = "playerLoad"
...
Sub OnContentChange()
    ...
    metadata = {duration : m.videoPlayer.duration, assetId : m.top.content.contentid , assetType: "external"}
